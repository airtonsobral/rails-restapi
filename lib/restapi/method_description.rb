...
module Restapi
  ...
  class MethodDescription
    ...
    def load_code_examples
      begin
        code_examples_yaml_hash = YAML.load_file(File.join(Restapi.configuration.code_examples_path, "#{@resource._id}.yml"))
        code_examples_hash = code_examples_yaml_hash[@resource._id.to_s][@method.to_s]
        code_examples = []
        code_examples_hash.each_pair do |language, code|
          code_examples << "Code example - #{language} \n#{code}"
        end
        code_examples
      rescue
      end
